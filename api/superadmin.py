                ...
                for task_index, task in enumerate(tasks):
                    ...
                    if has_subtasks:
                        ...
                        for subtask_index, subtask in enumerate(subtasks):
                            ...
                            # --- Frequency-based marking criteria and trigger_count logic ---
                            if subtask_task_type in ['Daily', 'Weekly']:
                                if subtask_task_type == 'Daily':
                                    trigger_count = (subtask_end_date - subtask_start_date).days + 1
                                else:
                                    trigger_count = ((subtask_end_date - subtask_start_date).days // 7) + 1
                                subtask['trigger_count'] = trigger_count
                                subtask['frequency_based_marking_criteria'] = {
                                    'fully_completed': round(marking_criteria['fully_completed'] / trigger_count, 2),
                                    'partially_completed': round(marking_criteria['partially_completed'] / trigger_count, 2),
                                    'incomplete': round(marking_criteria['incomplete'] / trigger_count, 2)
                                }
                            # --- End frequency-based logic ---
                        ...
                        # --- Frequency-based marking criteria and trigger_count logic for tasks without subtasks ---
                        task_type = task.get('task_type', 'Once')
                        if task_type in ['Daily', 'Weekly']:
                            if task_type == 'Daily':
                                trigger_count = (end_date - start_date).days + 1
                            else:
                                trigger_count = ((end_date - start_date).days // 7) + 1
                            task['trigger_count'] = trigger_count
                            task['frequency_based_marking_criteria'] = {
                                'fully_completed': round(marking_criteria['fully_completed'] / trigger_count, 2),
                                'partially_completed': round(marking_criteria['partially_completed'] / trigger_count, 2),
                                'incomplete': round(marking_criteria['incomplete'] / trigger_count, 2)
                            }
